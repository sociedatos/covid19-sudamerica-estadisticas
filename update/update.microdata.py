...
def get_iso3166(adm1_df, iso):
    ...
    adm1_index = map(
        lambda _: perkins.RE_PREFIX.sub('', _).strip(), adm1_index
    )
...
def download_peru(URL, sep=';', date_format='%Y%m%d', **kwargs):
    if not URL.startswith('/'):
        cdata = requests.get(URL, headers=perkins.DEFAULT_HEADERS, timeout=30, verify=False)
...
def get_latest_revision(URL):
    cdata = requests.get(URL, headers=perkins.DEFAULT_HEADERS, timeout=30)
    cdata = cdata.json()
...
def do_merge(result_dict):
    for key, df in result_dict.items():
        ...
        path = os.path.join(BASE_PATH, FILE_MAP[path[-1]])
        path = path + '.csv'
